# circumflex

[English version](./README.md)

🌿 It's Hacker News in your terminal

![circumflex](https://repo.x-cmd.io/circumflex.svg?lang=zh)

## 安装

```sh
x install circumflex
```

## 代码规模

合计: **32,002** 行代码（覆盖前 5 种语言、共 **254** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| Go | 31,052 | 4,060 | 8,987 | 235 |
| AsciiDoc | 277 | 0 | 125 | 1 |
| Bash | 269 | 98 | 59 | 1 |
| Html | 249 | 1 | 9 | 16 |
| Fish | 140 | 49 | 46 | 1 |

## 源代码

- **上游仓库**: <https://github.com/bensadeh/circumflex>
- **许可证**: MIT

## 发布

- **最新版本**: `5.0` (2026-07-29)
- **最近提交**: 2026-09-07
- **Release 含资产**: 6 个

## 流行度

- **Star**: 2,068 · **Fork**: 45 · **开放 issue**: 42 · **贡献者**: 9

## 累计统计

- **发布数**: 93 · **已合并 PR**: 143 · **开放 PR**: 1 · **已关闭 issue**: 42 · **开放 issue**: 0 · **提交数**: 2860

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-11 | 0 | 4 | 0 | 0 | 0 | 5 |
| last60d | 2026-07-12 | 1 | 7 | 0 | 0 | 0 | 140 |
| 90d | 2026-06-12 | 3 | 10 | 1 | 0 | 0 | 260 |
| last180d | 2026-03-14 | 8 | 17 | 1 | 6 | 0 | 504 |
| 360d | 2025-09-15 | 9 | 27 | 1 | 7 | 0 | 594 |
| last720d | 2024-09-20 | 10 | 55 | 1 | 16 | 0 | 628 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [checksums.txt](https://github.com/bensadeh/circumflex/releases/download/5.0/checksums.txt) | 500 B | `other` |
| [circumflex_5.0_darwin_amd64.tar.gz](https://github.com/bensadeh/circumflex/releases/download/5.0/circumflex_5.0_darwin_amd64.tar.gz) | 11.7 MiB | `native/darwin/x64` |
| [circumflex_5.0_darwin_arm64.tar.gz](https://github.com/bensadeh/circumflex/releases/download/5.0/circumflex_5.0_darwin_arm64.tar.gz) | 11.1 MiB | `native/darwin/arm64` |
| [circumflex_5.0_linux_386.tar.gz](https://github.com/bensadeh/circumflex/releases/download/5.0/circumflex_5.0_linux_386.tar.gz) | 10.8 MiB | `native/unknown` |
| [circumflex_5.0_linux_amd64.tar.gz](https://github.com/bensadeh/circumflex/releases/download/5.0/circumflex_5.0_linux_amd64.tar.gz) | 11.6 MiB | `native/linux/x64` |
| [circumflex_5.0_linux_arm64.tar.gz](https://github.com/bensadeh/circumflex/releases/download/5.0/circumflex_5.0_linux_arm64.tar.gz) | 10.7 MiB | `native/linux/arm64` |

## 发行版状态

在 [repology.org](https://repology.org/project/circumflex) 上共有 **12** 个发行版报告此项目。**4** 个 ✅ 已是最新上游版本，**7** 个 ⚠️ 使用旧版本。

| 发行版 | 版本 | 状态 |
|--------|------|------|
| Homebrew | `5.0` | ✅ latest |
| Nix unstable | `5.0` | ✅ latest |

## 改进这些数据

circumflex 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `circumflex` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/circumflex.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260910.yml` · 2026-09-10T21:16:14Z._
